...
export default function StudentLogin() {
  ...
  try {
    ...
    if (!roleData || roleData.role !== 'student') {
      // Security: If they aren't a student, sign them out immediately
      await supabase.auth.signOut();
      
      // We throw a generic error to avoid revealing role information to attackers
      throw new Error("Access denied. This portal is strictly for student accounts.");
    }
    ...
    toast({ 
      title: "Login failed", 
      description: error.message || "Invalid student email or password.", 
      variant: "destructive" 
    });
  } finally {
    setLoading(false);
      ...
      <Card className="w-full max-w-md border-blue-200 shadow-lg">
        <CardHeader className="text-center">
          ...
          <CardTitle className="font-display text-2xl text-blue-900">
            {mode === "login" ? "Student Portal" : "Reset Student Password"}
          </CardTitle>
          <CardDescription>
            {mode === "login"
        ...
        <CardContent>
          <form onSubmit={mode === "login" ? handleStudentLogin : handleForgotPassword} className="space-y-4">
