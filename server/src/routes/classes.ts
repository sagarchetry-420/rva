...
// GET /api/classes/simple — id and name only (for dropdowns) - MUST BE FIRST
...
// GET /api/classes/:id — get a single class (MUST BE LAST)
classesRouter.get('/:id', requireAuth, async (req, res) => {
  try {
    const supabase = createAdminClient();
    const { data, error } = await supabase
      .from('classes')
